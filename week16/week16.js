...
function makeFour() {
	//Ваш код
	const formsPracticumFive = document.forms;
	const formNames = [];
	for(let i = 0; i < formsPracticumFive.length; i++) {
		formNames.push(formsPracticumFive[i].getAttribute("name"));
	}
...
function makeEleven() {
	//Ваш код
	const formsEleven = document.forms.formOne;
	const selectEleven = formsEleven.firstSelect;
	let optionsValues = "";
	for(let i = 0; i < selectEleven.length; i++) {
		optionsValues += selectEleven[i].value + "\n";
	}
...
function makeTwelve() {
	//Ваш код
	const formsTwelve = document.forms.formTwo;
	const checkboxesTwelve = formsTwelve.querySelectorAll(".form__checkbox");
	let answerTwelve = "";
	for(let i = 0; i < checkboxesTwelve.length; i++) {
		answerTwelve += checkboxesTwelve[i].getAttribute("id") + "\n";
	}
	paragraphTwelve.textContent = answerTwelve;
}
...
function checkButton(e) {
	//Ваш код
	e.preventDefault();
	const formsThirteen = document.forms.lastForm;
	const radioThirteen = formsThirteen.fourthName;
	if(radioThirteen.checked) {
...
function checkOption() {
	//Ваш код
	const formsFourteen = document.forms.formOne;
	const selectFourteen = formsFourteen.firstSelect;
	selectFourteen.selectedIndex = 1;
	if(selectFourteen.value === selectFourteen[0].value) {
		paragraphFourteen.textContent = "Выбран первый вариант";
	} else if(selectFourteen.value === selectFourteen[1].value) {
...
function makeFifteen() {
	//Ваш код
	const formsFifteen = document.forms.formOne;
	const selectFifteen = formsFifteen.firstSelect;
	selectFifteen.selectedIndex = 0;
}
...
function makeSixteen() {
	//Ваш код
	const formsSixteen = document.forms.formTwo;
	const selectSixteen = formsSixteen.checkboxThree;
	selectSixteen.checked = true;
}
...
formOne.addEventListener('submit', function (event) {
	...
	//Ваш код
	const inputName = formOne.firstName;
	const inputEmail = formOne.firstEmail;
	if(inputName.value === "" && inputEmail.value === "") {
		document.getElementById('errorMessage').textContent = "Заполните поля с именем и с email";
	} else if(inputName.value === "") {
		document.getElementById('errorMessage').textContent = "Заполните поле с именем";
	} else if(inputEmail.value === "") {
		document.getElementById('errorMessage').textContent = "Заполните поле с email";
	} else {
...
const selectElement = document.forms.formOne.firstSelect; //Ваш код

selectElement.onchange = function () {
	//Ваш код
	if(selectElement.value === selectElement[0].value) {
		document.body.style.backgroundColor = "red";
	} else if(selectElement.value === selectElement[1].value) {
...
document.querySelector('.b-23').onclick = function (event) {
	if(formFour.fourthName.checked) {
		formFour.fourthButton.disabled = false;
		formFour.reset();
	} else {
		formFour.fourthButton.disabled = true;
	}
}
...
document.querySelector('.b-25').onclick = function () {
	...
	inputs.forEach(function (input) {
		...
		input.addEventListener('blur', function () {
			//Ваш код
			input.style.borderColor = this.defaultValue;
		});
	});
};
...
document.querySelector('.b-26').onclick = function (event) {
	...
	//Ваш код
	document.getElementById('result26').textContent = document.forms.formThree.thirdName.placeholder;
};
